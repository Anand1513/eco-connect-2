...
export class DatabaseStorage implements IStorage {
  ...
  async getAnalytics(): Promise<{
    ...
    const totalMeals = completedClaims.reduce((sum: number, claim: FoodClaim) => {
      const listing = allListings.find((l: FoodListing) => l.id === claim.foodListingId);
...
class MemoryStorage implements IStorage {
  ...
  private async seedDemoData() {
    ...
    const l1: FoodListing = {
      ...
      pickupTimeStart: two,
      pickupTimeEnd: four,
      location: "Green Bites, Pari Chowk (28.4743, 77.5030)",
      status: "available",
      createdAt: new Date(),
    };
